  ...
  const activityData = async (): Promise<any> => {
    try {
      ...
      if(type === 'video' || type === 'audio') {
        media = response.data.data[randomData].attributes.media.data.reduce((media: any, item: any,) => {
          if(item.attributes.url.includes("mp4") || item.attributes.url.includes("mp3")) media.data = baseUrl+item.attributes.url
          else media.thumbnail = baseUrl+item.attributes.url
          return media;
        }, {})
      } else {
        media = response.data.data[randomData].attributes.media.data.map((item: any) => baseUrl+item.attributes.url)
      }
      return { media, type }
